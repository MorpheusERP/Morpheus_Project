...
if ($id_Lote) {
    ...
    $sqlLote = "SELECT id_Lote, id_Usuario, nome_Usuario, data_Saida, valor_Lote
                FROM lote_saida
                WHERE id_Lote = ?";
    ...
    if ($stmtLote) {
        ...
        if ($resultLote->num_rows > 0) {
            ...
            $lote = [
                ...
                'data_Saida' => (new DateTime($dadosLote['data_Saida']))->format('d/m/Y'),
    ...
    // 2. Consulta para obter os produtos associados ao id_Lote na tabela `saida_produtos`
    $produtos = []; // Array para armazenar os produtos

    $sqlProdutos = "SELECT * FROM saida_produtos WHERE id_Lote = ?";
